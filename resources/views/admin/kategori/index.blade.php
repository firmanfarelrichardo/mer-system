{{--
|--------------------------------------------------------------------------
| Daftar Kategori Kesalahan (admin/kategori/index.blade.php)
|--------------------------------------------------------------------------
| Tabel daftar kategori kesalahan insiden dengan pencarian dan aksi CRUD.
|--------------------------------------------------------------------------
--}}

@extends('layouts.app')

@section('judul', 'Master Kategori Insiden — Sistem MER')
...
<div
    x-data='{
        ...
        bukaModalEdit(d) {
            this.mode   = "edit";
            this.editId = d.id;
            this.fields = { nama_kategori: d.nama_kategori };
            this.modalBuka = true;
        },

        get judul() {
            return this.mode === "buat" ? "Tambah Kategori Insiden" : "Edit Kategori Insiden";
        },

        get formAction() {
            return this.mode === "buat"
                ? "{{ route("admin.kategori.simpan") }}"
...
>
    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Master Kategori Insiden</h1>
            <p class="mt-1 text-sm text-slate-400">
                Kelola daftar kategori kesalahan untuk klasifikasi insiden.
            </p>
        </div>
    ...
    <div class="rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        @if ($daftarKategori->isEmpty())
            <div class="px-6 py-12 text-center">
                ...
                <p class="mt-3 text-sm text-slate-500">Belum ada kategori kesalahan terdaftar.</p>
                <button @click="bukaModalBuat()"
                ...
                <table class="w-full text-left text-sm">
                    <thead>
                        <tr class="border-b border-slate-100 bg-slate-50">
                            ...
                            <th class="px-5 py-3 font-semibold text-slate-500">Nama Kategori</th>
                            ...
                            <tr class="transition-colors hover:bg-slate-50/50">
                                ...
                                <td class="px-5 py-3 text-center">
                                    <div class="flex items-center justify-center gap-1">
                                        ...
                                        <button type="button"
                                                title="Edit Kategori"
                                                data-kategori='{{ $kategoriJson }}'
                                                @click="bukaModalEdit(JSON.parse($el.dataset.kategori))"
                                        ...
                                        {{-- Hapus --}}
                                        <form method="POST" action="{{ route('admin.kategori.hapus', $kategori->id) }}"
                                              onsubmit="return confirm('Hapus kategori {{ $kategori->nama_kategori }}? Aksi ini tidak dapat dibatalkan.')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
    ...
    <div x-show="modalBuka"
        ...
        <div x-show="modalBuka"
            ...
            <form method="POST" x-bind:action="formAction" class="px-6 py-5 space-y-4">
                ...
                {{-- Hidden: method override untuk PUT (edit) --}}
                <input type="hidden" name="_method" :value="mode === 'edit' ? 'PUT' : ''">

                {{-- Hidden: simpan state modal untuk recovery validasi --}}
                <input type="hidden" name="_modal_mode"    :value="mode">
                <input type="hidden" name="_modal_edit_id" :value="editId">

                {{-- Nama Kategori --}}
                <div>
                    <label for="modal_nama_kategori"
                    ...
                    <input type="text"
                           ...
                           name="nama_kategori"
                           x-model="fields.nama_kategori"
                           placeholder="Contoh: Kesalahan Prosedur"
                           autocomplete="off"
                           class="w-full rounded-lg border px-3 py-2 text-sm placeholder-slate-400
                                  focus:outline-none focus:ring-1
                                  @error('nama_kategori') border-red-400 focus:border-red-400 focus:ring-red-300
