{{--
|--------------------------------------------------------------------------
| Master Tipe Cedera (admin/master-form/tipe-cedera.blade.php)
|--------------------------------------------------------------------------
| Tabel daftar tipe cedera pasien dengan pencarian, toggle aktif, dan CRUD.
|--------------------------------------------------------------------------
--}}

@extends('layouts.app')

@section('judul', 'Master Tipe Cedera — Sistem MER')
...
<div
    x-data='{
        ...
        get judul() {
            return this.mode === "buat" ? "Tambah Tipe Cedera" : "Edit Tipe Cedera";
        },

        get formAction() {
            return this.mode === "buat"
                ? "{{ route("admin.tipe-cedera.simpan") }}"
                : "{{ url("admin/tipe-cedera") }}/" + this.editId;
        },
    }'
>
    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Master Tipe Cedera</h1>
            <p class="mt-1 text-sm text-slate-400">
                Kelola daftar tipe cedera pasien untuk formulir pelaporan kesalahan pengobatan.
            </p>
        </div>
    ...
    {{-- ================================================================
         FILTER & PENCARIAN
         ================================================================ --}}
    <form method="GET" action="{{ route('admin.tipe-cedera.index') }}"
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end">
            <div class="flex-1">
                ...
                <div class="relative">
                    ...
                    <input type="text" id="cari" name="cari" value="{{ $filter['cari'] ?? '' }}"
                           placeholder="Cari nama tipe cedera…"
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('admin.tipe-cedera.index') }}"
    ...
    <div class="rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        @if ($daftarData->isEmpty())
            <div class="px-6 py-12 text-center">
                ...
                <p class="mt-3 text-sm text-slate-500">Belum ada tipe cedera terdaftar.</p>
                <button @click="bukaModalBuat()"
                        class="mt-3 inline-block text-sm font-medium text-brand hover:underline">
                    + Tambah tipe cedera baru
                </button>
            </div>
        @else
                        ...
                        @foreach ($daftarData as $item)
                            ...
                            <tr class="transition-colors hover:bg-slate-50/50">
                                ...
                                <td class="px-5 py-3 text-center">
                                    <form method="POST" action="{{ route('admin.tipe-cedera.toggle-aktif', $item->id) }}" class="inline">
                                        @csrf
                                        @method('PATCH')
                                ...
                                <td class="px-5 py-3 text-center">
                                    <div class="flex items-center justify-center gap-1">
                                        ...
                                        <form method="POST" action="{{ route('admin.tipe-cedera.hapus', $item->id) }}"
                                              data-confirm="Hapus tipe cedera {{ addslashes($item->nama) }}? Aksi ini tidak dapat dibatalkan."
            ...
            <form method="POST" x-bind:action="formAction" class="px-6 py-5 space-y-4">
                ...
                <div>
                    <label for="modal_nama"
                        ...
                        Nama Tipe Cedera <span class="text-red-500">*</span>
                    </label>
                    <input type="text"
                           id="modal_nama"
                           name="nama"
                           x-model="fields.nama"
                           placeholder="Contoh: Luka Bakar"
                           autocomplete="off"
