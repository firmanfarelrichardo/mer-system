{{--
|--------------------------------------------------------------------------
| Master Faktor Penyebab (admin/master-form/faktor-penyebab.blade.php)
|--------------------------------------------------------------------------
| Tabel daftar faktor penyebab insiden dengan pencarian, toggle aktif, dan CRUD.
|--------------------------------------------------------------------------
--}}

@extends('layouts.app')

@section('judul', 'Master Faktor Penyebab — Sistem MER')
...
<div
    x-data='{
        ...
        get judul() {
            return this.mode === "buat" ? "Tambah Faktor Penyebab" : "Edit Faktor Penyebab";
        },

        get formAction() {
            return this.mode === "buat"
                ? "{{ route("admin.faktor-penyebab.simpan") }}"
                : "{{ url("admin/faktor-penyebab") }}/" + this.editId;
        },
    }'
>
    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Master Faktor Penyebab</h1>
            <p class="mt-1 text-sm text-slate-400">
                Kelola daftar faktor penyebab/kontribusi untuk formulir pelaporan kesalahan pcntl_errno.
            </p>
        </div>
    ...
    {{-- ================================================================
         FILTER & PENCARIAN
         ================================================================ --}}
    <form method="GET" action="{{ route('admin.faktor-penyebab.index') }}"
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end">
            <div class="flex-1">
                ...
                <div class="relative">
                    ...
                    <input type="text" id="cari" name="cari" value="{{ $filter['cari'] ?? '' }}"
                           placeholder="Cari nama faktor penyebab…"
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('admin.faktor-penyebab.index') }}"
    ...
    <div class="rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        @if ($daftarData->isEmpty())
            <div class="px-6 py-12 text-center">
                ...
                <p class="mt-3 text-sm text-slate-500">Belum ada faktor penyebab terdaftar.</p>
                <button @click="bukaModalBuat()"
                        class="mt-3 inline-block text-sm font-medium text-brand hover:underline">
                    + Tambah faktor penyebab baru
                </button>
            </div>
        @else
                        ...
                        @foreach ($daftarData as $item)
                            ...
                            <tr class="transition-colors hover:bg-slate-50/50">
                                ...
                                <td class="px-5 py-3 text-center">
                                    <form method="POST" action="{{ route('admin.faktor-penyebab.toggle-aktif', $item->id) }}" class="inline">
                                        @csrf
                                        @method('PATCH')
                                ...
                                <td class="px-5 py-3 text-center">
                                    <div class="flex items-center justify-center gap-1">
                                        ...
                                        <form method="POST" action="{{ route('admin.faktor-penyebab.hapus', $item->id) }}"
                                              data-confirm="Hapus faktor penyebab {{ addslashes($item->nama) }}? Aksi ini tidak dapat dibatalkan."
            ...
            <form method="POST" x-bind:action="formAction" class="px-6 py-5 space-y-4">
                ...
                <div>
                    <label for="modal_nama"
                        ...
                        Nama Faktor Penyebab <span class="text-red-500">*</span>
                    </label>
                    <input type="text"
                           id="modal_nama"
                           name="nama"
                           x-model="fields.nama"
                           placeholder="Contoh: Alergi Obat Tidak Tercatat"
                           autocomplete="off"
