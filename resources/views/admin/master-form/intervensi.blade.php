{{--
|--------------------------------------------------------------------------
| Master Tindakan Intervensi (admin/master-form/intervensi.blade.php)
|--------------------------------------------------------------------------
| Tabel daftar tindakan intervensi pasien dengan pencarian, toggle aktif, dan CRUD.
|--------------------------------------------------------------------------
--}}
...
<div
    x-data='{
        ...
        get judul() {
            return this.mode === "buat" ? "Tambah Tindakan Intervensi" : "Edit Tindakan Intervensi";
        },

        get formAction() {
            return this.mode === "buat"
                ? "{{ route("admin.intervensi.simpan") }}"
                : "{{ url("admin/intervensi") }}/" + this.editId;
        },
    }'
>
    ...
    {{-- ================================================================
         FILTER & PENCARIAN
         ================================================================ --}}
    <form method="GET" action="{{ route('admin.intervensi.index') }}"
        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end">
            <div class="flex-1">
                ...
                <div class="relative">
                    ...
                    <input type="text" id="cari" name="cari" value="{{ $filter['cari'] ?? '' }}"
                           placeholder="Cari nama tindakan intervensi…"
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('admin.intervensi.index') }}"
    ...
    {{-- ================================================================
         TABEL DAFTAR TINDAKAN INTERVENSI
         ================================================================ --}}
    <div class="rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        @if ($daftarData->isEmpty())
            <div class="px-6 py-12 text-center">
                ...
                <p class="mt-3 text-sm text-slate-500">Belum ada tindakan intervensi terdaftar.</p>
                <button @click="bukaModalBuat()"
                        class="mt-3 inline-block text-sm font-medium text-brand hover:underline">
                    + Tambah tindakan intervensi baru
                </button>
            </div>
        @else
                        ...
                        @foreach ($daftarData as $item)
                            ...
                            <tr class="transition-colors hover:bg-slate-50/50">
                                ...
                                <td class="px-5 py-3 text-center">
                                    <form method="POST" action="{{ route('admin.intervensi.toggle-aktif', $item->id) }}" class="inline">
                                        @csrf
                                        @method('PATCH')
            ...
            <form method="POST" x-bind:action="formAction" class="px-6 py-5 space-y-4">
                ...
                <div>
                    <label for="modal_nama"
                        ...
                        Nama Tindakan Intervensi <span class="text-red-500">*</span>
                    </label>
                    <input type="text"
                           id="modal_nama"
                           name="nama"
                           x-model="fields.nama"
                           placeholder="Contoh: Transfusi Darah"
                           autocomplete="off"
